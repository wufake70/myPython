...
class MakeAccount:
    ...
    def __init__(self, name, pwd, money_sum=0):
        ...
        # 字典内在存入用户信息
        # dic2[self.name] = {'姓名': self.name, '余额': self.money_sum, '密码': self.pwd}
        # print(dic2)
        # with open("/storage/emulated/0/Download/python/账户本.txt", "w") as fp:
        #     fp.write(str(dic2))
    ...
    def storage_money(self, money_1):
        ...
        # dic2[self.name] = {'姓名': self.name, '余额': self.money_sum, '密码': self.pwd}
        # with open("/storage/emulated/0/Download/python/账户本.txt", "w") as fp1:
        #     fp1.write(str(dic2))
    ...
    def draw_money(self, money_2):
        if self.money_sum >= money_2:
            ...
            # dic2[self.name] = {'姓名': self.name, '余额': self.money_sum, '密码': self.pwd}
            # with open("/storage/emulated/0/Download/python/账户本.txt", "w") as fp2:
            #     fp2.write(str(dic2))
...
def control():
    ...
    if c_1 == '1':
        d_1 = input('请输入存款金额：')
        dic[b_11].storage_money(float(d_1))
        print('=' * 10)
        return control()

    elif c_1 == '2':
        d_2 = input('请输入提款金额：')
        dic[b_11].draw_money(float(d_2))
        print('=' * 10)
        return control()

    elif c_1 == '3':
        print(f'您的余额为{dic[b_11].money_sum}')
        print('=' * 10)
        return control()
    elif c_1 == '4':
        del dic[b_11]
