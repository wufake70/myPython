...
class Crawl(object):
    ...
    def __init__(self):
        self.server_main = 'https://www.mmm131.com/xinggan/5775_4.html'
        # self.server_type = ''
        self.num = 23  # 自定义爬取页数
        self.EveryList = []
        self.PicNum = 0
            ...
            if isinstance(bf5, bs4.BeautifulSoup):  #########
                ...
                for i in range(len(self.EveryMainPics)):
                    ...
                    for eachLink in self.EveryDetailPics:
                        ...
                        try:
                            bf4 = BeautifulSoup(str(bf3.find_all('a', href="javascript:dPlayNext();")[0]),
                                                features='lxml')
                        except:
                            print('Error')
                            time.sleep(random.randint(1, 2))
                        ...
                        try:
                            response = requests.get(url=JPGLINK, headers=self.headers,
                                                    proxies=random.choice(self.proxies), timeout=20)
                            response.close()
